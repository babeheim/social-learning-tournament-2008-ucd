...
function [move, myRep]=woot_magic_trigger_pc_learn_50_50_no_lim(roundsAlive, myRepertoire, myHistory)
   ...
   %choose a move based on the highest magic trigger.  In a tie, pick the most recent action.
    ...
    if (myHistory(3,size(myHistory,2))~=myHistory(3,size(myHistory,2)-1)) || (myHistory(4,size(myHistory,2))<myHistory(4,size(myHistory,2)-1))
        for aa=1:size(myRepertoire,2) %for everything in the repetoir
            ...
            magic_trigger(aa)=(myRepertoire(2,aa)-est_mean_payoff)*((1-est_env_change)^(time_since_last(aa)+1)); %#ok<AGROW> %caculates the magic trigger
...
% decide if we should instead learn again - Plan C
if (roundsAlive > 10) && (exp_payoff < est_mean_payoff)
    if est_mean_payoff == 0
        move = 0;  %If the mean estimate is 0 [when does this happen?], you should observe a third time.  
    else
        choice=rand;
