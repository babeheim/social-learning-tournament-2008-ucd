...
if (size(myRepertoire, 2)==1) && (size(myHistory,2) > 1) && (roundsAlive > 1) && (myHistory(2,1)~=-1) %% This is for the case where there are two indentical observations: implying population is at a maximum
...
elseif size(myRepertoire, 2)<2   %%%if there are less than two moves in the repetoire learn another move
